...
def zero_pad(img,x=1,y=1):
    ...
    for r in img:
        ...
        for c in r:
            l.append(c)
        pad.append(l)
    pad  = [[0]*(len(pad[0]))]*y+pad+[[0]*(len(pad[0]))]*y
    return [[0]*x+e+[0]*x for e in pad]
...
def normalize(img):
    ...
    for r in range(len(img)):
        l = []
        for c in range(len(img[0])):
            l.append(float(img[r][c]-min_i)/(max_i-min_i)*255)
        norm.append(l)
    img = norm
    #raise NotImplementedError
    return img
...
def edge_magnitude(edge_x, edge_y):
    ...
    """
    edge_mag = []
    for r in range(len(edge_x)):
        t= []
        for c in range(len(edge_x[0])):
            t.append((edge_x[r][c]**2 + edge_y[r][c]**2)**0.5)
        edge_mag.append(t)
